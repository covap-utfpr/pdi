...
(ref:areasPDI) Processos Computacionais com Imagens [@velho2009, p. 2] **Precisa explicar a figura aqui em sau descrição e também no texto. Veja as figuas do full text nesse link: https://www.seer.ufrgs.br/rita/article/view/11477**.
...
Table: (\#tab:processos) Processos Computacionais com Imagens. **Sempre é preciso descrever tabelas e figuras de forma rápida em seus cabeçalhos ou roda-pés. Os detalhes ficam no texto.**
...
Baseado nesses níveis, iremos considerar que PDI atua nos primeiros dois níveis, ou seja, envolve o pré-processamento e processos de extração de elementos de imagens até a detecção de componentes individuais. Como o foco desta parte do material é o Processamento de Imagens Digitais (PDI), estes dois níveis serão apresentados com detalhes nos próximos tópicos, mas primeiro vejamos alguns exemplos de aplicações de PDI. **(acho que está última parte depois da vírgula pode ser retirada invertendo os tópicos 1.2 e 1.3)** 
...
Outra área que também faz uso extensivo de PDI e impulsionou seu desenvolvimento é a área médica. Nesse contexto, o uso de imagens auxiliou no diagnóstico de doenças através de exames visuais como os de raio-x  [@gonzalez2010, p. 4].
...
Para compreender as relações entre as etapas de processamento e análise de imagens podemos observar na Figura \@ref(fig:etapasPDI) uma sequência dos principais passos utilizados em uma aplicação de PDI. Nesta parte do material nos deteremos aos conteúdos de processamento, desde a aquisição de imagens, pré-processamento até segmentação.

(ref:etapasPDI) Etapas de aplicação de PDI [@pedrini2008, p. 4]. **(novamente, descrrever rapidamente as partes da figura aqui. Veja isso em todas as figuras e  tabelas)**
...
Vale ressaltar que essas etapas não são fixas e podem ser modificadas, sendo que uma base de conhecimentos é importante para orientar em uma aplicação específica [@pedrini2008, p. 4]. **(este parágrafo está vago ou fora de lugar. Escreva um parágrafo bem rápido descrevendo a figura e termine dizendo que cada faze será detalhada a seguir e aí vem cada que etá na sequencia)**

- **Aquisição da imagem**

    Captura a imagem por meio de um dispositivo ou sensor e a converte em uma imagem digitalizada [@pedrini2008, p. 3]. Podemos citar como exemplo as câmeras fotográficas, tomógrafos médicos, satélites e scanners. Na Figura \@ref(fig:aquisicao), temos um exemplo de aquisição de imagens do satélite Landsat. Neste caso estão identificadas as bandas vermelha, verde e azul visíveis e a faixa do infravermelho próximo. Os detalhes sobre a aquisição de imagens serão discutidos no tópico **Formação de Imagem**. *(como é hipertexto, pode ser um link, certo?)*

(ref:aquisicao) Exemplo de imagem adquirida por satélite.(a) a (d) mostram quatro imagens espectrais de satélite da cidade de Washington, D.C., banda vermelha, verde e azul visíveis e o infravermelho próximo, respectivamente. (e) Imagem colorida como combinação RGB de (a), (b) e (c). (f) Imagem colorida obtida pela combinação de (b), (c) e (d)[@gonzalez2010, p. 279]. **(A imagem foi copiada? não está bom de ver as letras indicadas na legenda. precisa resolver isso)**
...
(ref:preProcessamento) Subtração de imagens para realce de diferenças.(a) Imagem da área de Washington D.C em infravermelho. (b) Resultado ao zerar o bit menos significativo de todos os pixels de (a). (c) Diferença entre as duas imagens ajustada para a faixa [0, 255], sendo que valores em preto (0) indicam pontos nos quais não há nenhuma diferença [@gonzalez2010, p. 49]. **(Não entendi bem esta figura... novamente as letras não estão bem visíveis e porque zerar o bit menos significativo. O texto não fala nada sobre isso...)**
...
(ref:segmentacao) Extração de características de uma imagem segmentada.(a) Imagem na banda infravermelha da área de Washington, D.C. (b) Segmentação da imagem por limiarização. (c) O maior componente conexo de (b). Técnica de representação por esqueleto de (c) [@gonzalez2010, p. 544]. **(O que é um coponente conexo? e o que é o esqueleto?)**
...
- **Representação e Descrição**
    ...
    Nesta fase o objetivo é armazenar e manipular objetos de interesse extraídos da imagem. O processo de descrição visa a extração de características para discriminar classes de objetos [@pedrini2008, p. 4]. Na Figura \@ref(fig:analise), o objetivo é determinar o tamanho das ramificações do rio e para isto considerou-se que o tamanho de cada ramificação no esqueleto seria uma boa aproximação [@gonzalez2010, p. 545]. O esqueleto é uma representação do rio, e seus elementos são discriminados dentro do maior componente conexo da imagem segmentada. **(de novo... faltam informações antes para sabermos o que é um compnente conexo e também o esqueleto)** 


- **Reconhecimento e Interpretação**
	
    Essa etapa examina as informações produzidas na etapa anterior e classifica cada objeto como sendo de interesse ou não, atribuindo significado ao conjunto de objetos reconhecidos pelos rótulos [@pedrini2008, p. 3]. Uma aplicação de análise de imagens inclui a classificação de áreas em uma imagem multiespectral como na Figura \@ref(fig:analise). Neste exemplo utilizou-se o método bayesiano, em que cada pixel da imagem foi avaliado em relação a três classes (água, desenvolvimento urbano e vegetação). Na Figura \@ref(fig:analise), pontos pretos representam pontos classificados incorretamente, enquanto pontos brancos foram classificados corretamente [@gonzalez2010, p. 579].

(ref:analise) Classificação bayesiana em uma imagem multiespectral. Resultado (em branco) da classificação na classe água, desenvolvimento urbano e vegetação, da esquerda para a direita [@gonzalez2010, p. 579]. **(Coloque (a), (b) e (c))**
...
Acrescente um parágrafo aqui introduzindo a seção seguinte que será a atual 1.2. Talvez o parágrafo lá de cima que parece estar fora de lugar fique bem aqui com certos ajustes...
