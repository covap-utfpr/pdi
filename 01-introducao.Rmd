...
Essa etapa busca realizar mudanças e ajustes na imagem visando melhorar seu uso nas etapas futuras [@pedrini2008, p. 3]. Como exemplo temos casos onde não precisamos das cores de uma imagem, podendo então realizar a conversão para *grayscale* (tons de cinza), ou precisamos gerar imagens coloridas. Além disso, podemos realizar cortes ou realces, isolando somente a parte de maior interesse na imagem utilizando algumas das técnicas apresentadas no Capítulo [Transformações Geométricas](#transformacoesGeometricas) e no Capítulo [Transformações Radiométricas](#transformacoesRadiometricas). Na Figura \@ref(fig:placaEletronicaMediana) se destaca uma das abordagens recorrentes no tratamento de imagem, a atenuação de ruídos e a suavização, utilizando principalmente alguns dos filtros do Capítulo [Filtros Digitais](#filtros).

(ref:placaEletronicaMediana) Remoção de ruído sal e pimenta por meio do filtro de Mediana - No Capítulo [Filtros Digitais](#filtros) serão apresentados alguns filtros como este que tratam as imagens, por exemplo, revomendo ruídos, aumentando o realce ou suavizando[@gonzalez2010, p. 103].

```{r placaEletronicaMediana, echo=FALSE, fig.asp=.7, fig.width= 4, fig.cap='(ref:placaEletronicaMediana)', fig.align='center', out.width='85%'}
...
Ao individualizar estes componentes da imagem por meio de diferentes detectores, como na Figura \@ref(fig:doghubble) em que se utilizou um detector de Blob, as informações assumem um formato mais adequado para o processamento computacional. 

(ref:doghubble) Resultado da detecção de *blobs* com DoG para identificação de estrelas - Outros detectores no Capítulo [Segmentação](#segmentacao) podem identificar pontos, linhas, quinas, círculos, e objetos de outros formatos como estas estrelas.

```{r doghubble, echo=FALSE, fig.asp=.7, fig.width= 4, fig.cap='(ref:doghubble)', fig.align='center', out.width='35%'}
knitr::include_graphics(rep('imagens/06-segmentacao/dog_hubble.jpg'))
```

Outro exemplo da separação de elementos de uma imagem também está no Capítulo [Separação de Fundo](#separacaoFundo), em que modelos são utilizados para detectar o que é o *background*, ou plano de fundo, e o que é o objeto de interesse (*foreground*) como na Figura \@ref(fig:indoorBGmodeling).
...
```{r indoorBGmodeling, echo=FALSE, fig.asp=.7, fig.width= 4, fig.cap='(ref:indoorBGmodeling)', fig.align='center', out.width='50%'}
knitr::include_graphics(rep('imagens/08-modelagemBackground/indoorBGmodeling.png'))
```

- Análise 

À parte de Análise atribuímos todos os processos que fazem uso da informação visual para compor um sistema de aplicação na área de Visão Computacional. Atualmente é mais comum ver estes processos englobados dentro de modelos de aprendizado de máquina mais genéricos como em redes neurais, capazes por exemplo, de classificar, identificar e rastrear objetos em diferentes situações. No Capítulo [Deep Learning em Visão Computacional](#deepLearning), assim como na Figura \@ref(fig:differentscnn) é possível se ter uma pequena noção de como as redes neurais, em particular as redes convolucionais (CNN), enxergam as imagens e como desagregam suas informações para disponibilizar resultados satisfatórios no campo da VC. 
...
```{r differentscnn, echo=FALSE, fig.asp=.7, fig.width= 4, fig.cap='(ref:differentscnn)', fig.align='center', out.width='80%'}
