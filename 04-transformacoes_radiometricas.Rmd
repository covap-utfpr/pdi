...
No próximo passo, todos os valores de $T_2(z_q)$ são calculados da mesma forma que $T_1(r)$, Equação \@ref(eq:eqEqualizHistDiscret) de equalização. Os resultados desta segunda etapa estão na Figura \@ref(fig:tabelahistograma2)  e a função de transformação é esboçada na Figura \@ref(fig:histograma3)(c).
...
No terceiro passo, sendo $G(z_q) = T_2(z_q)$, verifica-se o menor valor de $z_q$ na Figura \@ref(fig:histograma3)(c) de modo que $G(z_q)$ seja o mais próximo possível de $s_k$ . Isso é feito para todos os valores de $s_k$ para criar os mapeamentos necessários de $s$ em $z$, Figura \@ref(fig:tabelahistograma4). A Figura \@ref(fig:tabelahistograma3) mostra os resultados do gráfico de \@ref(fig:histograma3)(c) em uma tabela.
...
Na última etapa, mapea-se cada intensidade da imagem do histograma equalizado, $p_s(s)$, em uma intensidade correspondente na imagem do histograma especificado recém-criado, $p_z(z)$, conforme ilustrado no mapeamento na Figura \@ref(fig:tabelahistograma4). O histograma resultante da especificação é esboçado na Figura \@ref(fig:histograma3)(d).
